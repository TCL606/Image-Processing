function [reveal_info, reveal_img] = dct_reveal_2(dc_code, ac_code, img_height, img_width, hide_idx, DCTAB, ACTAB, QTAB)
    ...
    total_bits = bitand(int32(whole_cof), int32(ones(size(whole_cof))));
    reveal_info = zeros(length(hide_idx), size(total_bits, 2));
    for i = 1: 1: size(total_bits, 2)
        reveal_info(:, i) = total_bits(hide_idx, i);
    end
    reveal_info = int32(reveal_info);
