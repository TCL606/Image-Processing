function [reveal_info, reveal_img] = dct_reveal_3(dc_code, ac_code, img_height, img_width, DCTAB, ACTAB, QTAB)
    ...
    reveal_info = zeros(1, size(whole_cof, 2));
    for i = 1: 1: length(reveal_info)
        for j = size(whole_cof, 1): -1: 1
            if whole_cof(j, i) ~= 0
                reveal_info(i) = whole_cof(j, i);
                break;
            end
        end
    end
